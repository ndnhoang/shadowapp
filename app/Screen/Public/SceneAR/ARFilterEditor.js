...
import { StyleSheet, CameraRoll, Image, Share, TouchableOpacity, Platform } from 'react-native';
import { Container, Content, Text, Icon, View, Toast, Footer, FooterTab } from 'native-base'
                            ...
                            <View collapsable={false} style={{ flex: 1, width: '100%', height: '100%' }} >
                                ...
                                <View ref={(ref) => { this.viewRef = ref; }} collapsable={false} style={{ flex: 1, position: 'absolute', width: '100%', height: '100%' }}>
                                    ...
                                    <View style={{ position: 'absolute', flex: 1, width: '100%', height: '100%' }}>
                                        <ScalableImage style={layerStyle.logo} width={layerStyle.logo.width} resizeMode={"contain"} source={require('@SFAsset/ShadowFactory(black).png')} />
                                    </View>
                                </View>
                                ...
                                <Footer style={[Style.greyTopLine, { position: 'absolute', bottom: 0, backgroundColor: '#fcfcfc' }]}>
                                    <FooterTab>
                                        <TouchableOpacity style={[Style.flex, Style.layoutCenter, { backgroundColor: '#fcfcfc' }]} onPress={this._OnRetakePressed}>
                                            ...
                                            <Text style={Style.SFText[i18n.language]}>{t('arfiltereditor.retake')}</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={[Style.flex, Style.layoutCenter, { backgroundColor: '#fcfcfc' }]} onPressOut={this.ScreenCapture}>
                                            ...
                                            <Text style={Style.SFText[i18n.language]}>{t('arfiltereditor.save')}</Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity style={[Style.flex, Style.layoutCenter, { backgroundColor: '#fcfcfc' }]} onPressOut={this._OnSharePressed}>
                                            ...
                                            <Text style={Style.SFText[i18n.language]}>{t('arfiltereditor.share')}</Text>
                                        </TouchableOpacity>
                                    </FooterTab>
                                </Footer>
                            </View>
    ...
    _share(uri) {
        ...
        if (Platform.OS === 'ios') {
            CameraRoll.saveToCameraRoll(uri).then(newURI => {
                ...
                console.log('=========== Captured: ' + uri);
                const msgToShare = { message: '', title: 'Shadow Factory', url: 'file://' + uri }

                Share.share(msgToShare, { dialogTitle: 'Shadow Factory' })
